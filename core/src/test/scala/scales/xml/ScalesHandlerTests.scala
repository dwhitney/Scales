...
class ScalesHandlerTests extends Spec with MustMatchers{
	...
	describe("A ScalesHandler"){
		...
		it("must parse component tags"){
			val xml = """
				<blah xmlns:s='http://www.scales-framework.org'>
		...
		it("must ignore the <s:ignore xmlns:s='http://www.scales-framework.org'></s:ignore> tag"){
			val xml = """
				<s:ignore xmlns:s="http://www.scales-framework.org">
					<h1>Hello, World!</h1>
				</s:ignore>
			"""
			//"
			val buffer = ScalesHandler.parse(xml)
			buffer.toString().trim() must equal("<h1 xmlns:s=\"http://www.scales-framework.org\">Hello, World!</h1>")
		}
	
		it("must parse even if the correct namespace for components isn't declared"){
			val xml = "<html xmlns:s=\"http://www.scales-framework.org\"><body><s:component class=\"foo.Bar\"/></body></html>"
			val buffer = ScalesHandler.parse(xml)
			buffer.toString.trim() must equal("<html xmlns:s=\"http://www.scales-framework.org\"><body>{foo.Bar(request, response)}</body></html>")
		}
		...
		it("must parse and produce a NodeSeq"){
			...
			*/
			...
			buffer.toString.trim() must equal("<head xmlns:s=\"http://www.scales-framework.org\"></head><body></body>")
		}
		...
		it("must replace <s:code/> with {} "){
			...
			*/
			...
			val buffer = ScalesHandler.parse(nodeSeq)
			buffer.toString.trim() must equal("<head xmlns:s=\"http://www.scales-framework.org\">{4 + 4}</head>")
		}
