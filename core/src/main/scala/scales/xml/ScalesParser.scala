/**
Much of this was inspired by the AltParser from the lift framework
...
object ScalesParser {
	...
	val inlineTags: Set[String] = Set("base", "meta", "link", "hr", "br","param", "img", "area", "input", "col" )

	def toXML(s: String): String = toXML(s, false, false, false)
	def toXML(x: Node): String = toXML(x, false, false, false)

	def toXML(s: String, stripComment: Boolean, convertAmp: Boolean, ieMode: Boolean): String = {
		val x: NodeSeq = XML.loadString(s)
		val sb = new StringBuilder(50000)
		x.foreach{ toXML(_, TopScope, sb, stripComment, convertAmp, ieMode)}
		sb.toString
	}

	def toXML(n: Node, stripComment: Boolean, convertAmp: Boolean, ieMode: Boolean): String = {
		val sb = new StringBuilder(50000)
	...
	*/
	...
	def toXML(x: Node, pscope: NamespaceBinding, sb: StringBuilder, stripComment: Boolean, convertAmp: Boolean, ieMode: Boolean): Unit = {
		x match {
		 	case c: Comment if !stripComment => c.toString(sb)
	
			case er: EntityRef if convertAmp => HtmlEntities.entMap.get(er.entityName) match {
			...
			case e: Elem if !ieMode && ((e.child eq null) || e.child.isEmpty) && inlineTags.contains(e.label) =>
				sb.append('<')
				e.nameToString(sb)
			...
			case e: Elem if ieMode && ((e.child eq null) || e.child.isEmpty) && ieBadTags.contains(e.label) =>
				sb.append('<')
				e.nameToString(sb)
				if (e.attributes ne null) e.attributes.toString(sb)
				e.scope.toString(sb, pscope)
	...
	* @param stripComment ...
	*/
	def sequenceToXML(children: Seq[Node], pscope: NamespaceBinding, sb: StringBuilder, stripComment: Boolean, convertAmp: Boolean, ieMode: Boolean): Unit = {
